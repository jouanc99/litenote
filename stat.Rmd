...
BMI = 體重(kg)/身高(m)^2
```{r}
dt<-cbind(stat_1$WHD010,stat_1$WHD020) #算今年bmi
dt_1_1<-subset(dt,dt[,1]<=84 & dt[,2]<=600,na.rm=TRUE)
dt_1_1_1<-dt_1_1[,1]*0.0254 #m
dt_1_2_1<-dt_1_1[,2]*0.45 #kg
bmi_now<-dt_1_2_1/(dt_1_1_1^2)
summary(bmi_now)

dt1<-cbind(stat_1$WHD010,stat_1$WHD050) #算前一年bmi
dt_2_1<-subset(dt1,dt1[,1]<=84 & dt[,2]<=600,na.rm=TRUE)
dt_2_1_1<-dt_2_1[,1]*0.0254 #m
dt_2_2_1<-dt_2_1[,2]*0.45 #kg
bmi_lastyr<-dt_2_2_1/(dt_2_1_1^2)
summary(bmi_lastyr)
...
```{r}
dif<-bmi_lastyr- bmi_now
paste0("large weight loss: ",length(as.factor(subset(dif,dif>=5))),"人")
paste0("mild weight loss: ",length(as.factor(subset(dif,dif<5& dif>=2))),"人")
paste0("you are a big loser: ",length(as.factor(subset(dif,dif<2))),"人")
